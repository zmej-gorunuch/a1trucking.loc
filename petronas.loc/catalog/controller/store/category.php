...
class ControllerProductStoreCategory extends Controller
{
	public function index()
	{
		$this->load->language('product/store_category');
		...
		if ($category_info) {
			...
			$data['heading_subtitle'] = $category_info['subname'];
			...
			$data['text_product_line'] = $this->language->get('product_line');
			$data['text_flip_down'] = $this->language->get('flip_down');
			...
			foreach ($results as $result) {
				...
				$data['categories'][] = array(
					...
					'image' => $this->model_tool_image->resize($result['image'], 467, 389)
				);
			}

			$top_category_id = $top_category_info = $this->model_catalog_category->getCategories(0)[0]['category_id'];
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 500, 320);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png',
						$this->config->get($this->config->get('config_theme') . '_image_product_width'), 500, 320);
				}
				...
				$category_info = $this->model_catalog_category->getCategory($product_category[0]['category_id']);

				if ($category_info['parent_id']) {
					$product_category = $this->model_catalog_category->getCategory($category_info['parent_id'])['name'];
				} else {
					$product_category = null;
				}

				$data['text_label_new'] = $this->language->get('text_label_new');
				if(strtotime($result['date_available']) > (time() - (60*60*24*30) )){
					$is_new = true;
				} else {
					$is_new = false;
				}
				...
				$data['text_reviews'] = sprintf($this->language->get('text_reviews'), $result['reviews']);
			}
			...
			$data['sorts'][] = array(
				...
				'href' => $this->url->link('product/store_category',
			...
			$data['sorts'][] = array(
				...
				'href' => $this->url->link('product/store_category',
					'path=' . $this->request->get['path'] . '&sort=p.date_added&order=DESC' . $url)
			);
...
//			);
			...
			if ($page == 1) {
				$this->document->addLink($this->url->link('product/category', 'path=' . $category_info['category_id'],
					true), 'canonical');
			} elseif ($page == 2) {
				$this->document->addLink($this->url->link('product/category', 'path=' . $category_info['category_id'],
					true), 'prev');
				...
				$this->document->addLink($this->url->link('product/category',
					'path=' . $category_info['category_id'] . '&page=' . ($page - 1), true), 'prev');
			}

			if ($limit && ceil($product_total / $limit) > $page) {
				$this->document->addLink($this->url->link('product/category',
					'path=' . $category_info['category_id'] . '&page=' . ($page + 1), true), 'next');
			}
			...
			$data['footer'] = $this->load->controller('common/footer_white');
			$data['header'] = $this->load->controller('common/header_white');
			...
			$this->response->setOutput($this->load->view('product/store_category', $data));
