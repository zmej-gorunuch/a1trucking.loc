	...
	<div class="row"><?php echo $column_left; ?>
		...
		<div id="content" class="<?php echo $class; ?>"><?php echo $content_top; ?>
			...
			<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
				...
				<div class="buttons">
					<div class="pull-right">
						<input class="btn btn-primary" type="submit" onclick="return regContactForm();"
			...
			<?php echo $content_bottom; ?></div>
                ...
                <script>
                    function regContactForm(e) {
                        let email = $('#input-email').val();
                        let name = $('#input-name').val();
                        let message = $('#input-enquiry').val();
                        $('form').on('submit', function (e) {
                            if (email && name && (name.length >= 3) && message && (message.length >= 10)) {
                                e.preventDefault();
                                $.ajax({
                                    url: 'index.php?route=extension/module/contact_form/add',
                                    type: 'post',
                                    ...
                                    success: function (json) {
                                        if (json.status === true) {
                                            location = '<?php echo $action; ?>/success';
                                        }
                                    }
                                })
                            } else {
